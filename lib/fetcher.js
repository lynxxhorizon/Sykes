...
exports.modStick = (media, ilhansykes, hann, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './media/data.exif', media, '-o', out])
        .on('exit', () => {
            ilhansykes.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: hann})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        ilhansykes.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: hann })
        fs.unlinkSync(media)
    }
}
